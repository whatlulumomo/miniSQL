...
int CatalogManager::getIndexType(string indexName)
{
    ...
    if (btmp )
    {
        ...
        IndexInformation * i = (IndexInformation *)addressBegin;
        for(int j = 0 ;j<(bm.get_usingSize(*btmp)/sizeof(IndexInformation));j++)
        {
...
int CatalogManager::getAllIndex(vector<IndexInformation> * indexs)
{
    fileNode *ftmp = bm.getFile("Indexs");
    blockNode *btmp = bm.getBlockHead(ftmp);
    if (btmp )
...
int CatalogManager::addIndex(string indexName,string tableName,string Attribute,int type)
{
    fileNode *ftmp = bm.getFile("Indexs");
    blockNode *btmp = bm.getBlockHead(ftmp);
    IndexInformation i(indexName,tableName,Attribute,type);
    while (true)
    {
        if (btmp == NULL)
        {
        ...
        if (bm.get_usingSize(*btmp) <= bm.getBlockSize() - sizeof(IndexInformation))
        {

            char* addressBegin;
            addressBegin = bm.get_content(*btmp) + bm.get_usingSize(*btmp);
            memcpy(addressBegin, &i, sizeof(IndexInformation));
            bm.set_usingSize(*btmp, bm.get_usingSize(*btmp) + sizeof(IndexInformation));
            bm.set_dirty(*btmp);
...
int CatalogManager::dropIndex(string index)
{
    ...
    if (btmp)
    {
        ...
        IndexInformation * i = (IndexInformation *)addressBegin;
        int j = 0;
        for(j = 0 ;j<(bm.get_usingSize(*btmp)/sizeof(IndexInformation));j++)
        {
        ...
        for(;j<(bm.get_usingSize(*btmp)/sizeof(IndexInformation)-1);j++)
        {
            (*i) = *(i + sizeof(IndexInformation));
            i ++;
        }
        bm.set_usingSize(*btmp, bm.get_usingSize(*btmp) - sizeof(IndexInformation));
        bm.set_dirty(*btmp);
        ...
        for(i =0;i<size;i++)
        {
            if(a->name == AttributeName)
            {
                ...
                else
                {
                    ...
                    string temp = "revoke unknown index: ";
                    temp += indexName + " on " + tableName + "!";
                    Table::text->append(QString::fromStdString(temp));
                    cout<<"Attribute: "<<AttributeName<<" on table "<<tableName<<" has index: "<<a->index<<"!"<<endl;
                    temp = "Attribute: ";
                    temp += AttributeName + " on table "+tableName+" has index: " + a->index + "!";
                    Table::text->append(QString::fromStdString(temp));
                }
                break;
            }
...
int CatalogManager::deleteValue(string tableName, int deleteNum)
{
    ...
    if (btmp)
    {
        ...
        if((*recordNum) <deleteNum)
        {
            ...
            Table::text->append("error in CatalogManager::deleteValue");
            return 0;
        }
        else
            (*recordNum) -= deleteNum;
    ...
    if (btmp)
    {
        ...
        for(int i =0;i<size;i++)
        {
            ...
            else
            {
                ...
                Table::text->append("Catalog data damaged!");
                return 0;
            }
            a ++;
        }
...
int CatalogManager::calcuteLenth2(int type){
    ...
    else if (type == Attribute::TYPE_FLOAT)
    {
        ...
        return (int) sizeof(char); // Note that the type stores in Attribute.h
    }
}
